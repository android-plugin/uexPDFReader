...
//        setContentView(R.layout.plugin_pdf_tap_zones_config);
        setContentView(EUExUtil.getResLayoutID("plugin_pdf_tap_zones_config"));
...
//        actionsAdapter = new ActionsAdapter(getContext(), R.array.list_actions_ids, R.array.list_actions_labels);
        actionsAdapter = new ActionsAdapter(getContext(), ResUtils.getResArrayId("plugin_pdf_list_actions_ids") ,ResUtils.getResArrayId("plugin_pdf_list_actions_labels") );

//        stList = (Spinner) this.findViewById(R.id.tapZonesConfigSingleAction);
        stList = (Spinner) this.findViewById(EUExUtil.getResIdID("tapZonesConfigSingleAction"));
...
//        dtList = (Spinner) this.findViewById(R.id.tapZonesConfigDoubleAction);
        dtList = (Spinner) this.findViewById(  EUExUtil.getResIdID("tapZonesConfigDoubleAction"));
...
//        ltList = (Spinner) this.findViewById(R.id.tapZonesConfigLongAction);
         ltList = (Spinner) this.findViewById(EUExUtil.getResIdID("tapZonesConfigLongAction"));
...
//        tftList = (Spinner) this.findViewById(R.id.tapZonesConfigTwoFingerAction);
        tftList = (Spinner) this.findViewById(EUExUtil.getResIdID("tapZonesConfigTwoFingerAction"));
...
//        regionList = (Spinner) this.findViewById(R.id.tapZonesConfigRegions);
        regionList = (Spinner) this.findViewById(EUExUtil.getResIdID("tapZonesConfigRegions"));
...
//        actions.connectViewToAction(R.id.tapZonesConfigReset);
        ...
        actions.connectViewToAction(EUExUtil.getResIdID("tapZonesConfigClear"));
        actions.connectViewToAction( EUExUtil.getResIdID("tapZonesConfigDelete"));
        actions.connectViewToAction(EUExUtil.getResIdID("tapZonesConfigReset"));
	...
	final static int id_tapZonesConfigReset;
	final static int id_tapZonesConfigClear;
	final static int id_tapZonesConfigDelete;
	static 
	{
		id_tapZonesConfigReset= getId("tapZonesConfigReset");
		id_tapZonesConfigClear= getId("tapZonesConfigClear");
		id_tapZonesConfigDelete= getId("tapZonesConfigDelete");
	}
...
//	@ActionMethod(ids = R.id.tapZonesConfigDelete)
	...
	private List<RegionWrapper> wraps(final List<Region> list) {
		final List<RegionWrapper> res = new ArrayList<RegionWrapper>(
				list.size());
	...
	private final class RegionSelectionListener implements
	...
	private final class ActionSelectionListener implements
			OnItemSelectedListener {
		...
		public void onItemSelected(final AdapterView<?> parent,
				final View view, final int position, final long id) {
			if (wrapper != null) {
				final Integer actionId = ActionEx.getActionId(actionsAdapter
						.getActionId(position));
				if (actionId != null) {
					wrapper.r
							.setAction((Touch) parent.getTag(), actionId, true);
				} else {
...
//			super(context, R.layout.plugin_pdf_list_item, R.id.list_item, objects);
//			setDropDownViewResource(R.layout.plugin_pdf_list_dropdown_item);
			
			super(context,EUExUtil.getResLayoutID("plugin_pdf_list_item") ,EUExUtil.getResIdID("list_item") , objects);
			setDropDownViewResource(EUExUtil.getResLayoutID("plugin_pdf_list_dropdown_item"));
		}
	}
	...
	public static final int getId(String key)
	{
		final int id = EUExUtil.getResIdID(key);
		return id;
	}
}
