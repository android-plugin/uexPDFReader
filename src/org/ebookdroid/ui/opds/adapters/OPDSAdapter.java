...
//        final ViewHolder holder = BaseViewHolder.getOrCreateViewHolder(ViewHolder.class, R.layout.plugin_pdf_opdsitem, view,
//                parent);
        final ViewHolder holder = BaseViewHolder.getOrCreateViewHolder(ViewHolder.class,  EUExUtil.getResLayoutID("plugin_pdf_opdsitem"), view,
                parent);
...
//                holder.imageView.setImageResource(R.drawable.plugin_pdf_opds_item_feed);
            	holder.imageView.setImageResource( EUExUtil.getResDrawableID("plugin_pdf_opds_item_feed"));
...
//                    holder.imageView.setImageResource(R.drawable.opds_item_book);
                     holder.imageView.setImageResource(EUExUtil.getResDrawableID("opds_item_book"));
...
//        b.show();
        ...
        final ActionDialogBuilder b = new ActionDialogBuilder(context, actions);
        b.setTitle(EUExUtil.getResStringID("plugin_pdf_opds_error_title"));
        b.setMessage(EUExUtil.getResStringID("plugin_pdf_opds_error_msg"), msg);
        if (pbAction != EUExUtil.getResIdID("actions_no_action")) {
            b.setPositiveButton(pbLabel, pbAction, new Constant("info", result));
            b.setNegativeButton();
        } else {
            b.setPositiveButton(pbLabel, pbAction);
...
//        final View view = LayoutInflater.from(context).inflate(R.layout.plugin_pdf_opds_auth_dlg, null);
...
//        final EditText editPassword = (EditText) view.findViewById(R.id.opds_auth_editPassword);
        ...
        final View view = LayoutInflater.from(context).inflate(  EUExUtil.getResLayoutID("plugin_pdf_opds_auth_dlg"), null);
        final EditText editUsername = (EditText) view.findViewById( EUExUtil.getResIdID("opds_auth_editUsername"));
        final EditText editPassword = (EditText) view.findViewById( EUExUtil.getResIdID("opds_auth_editPassword"));
...
//        b.show();
        ...
        b.setTitle(EUExUtil.getResStringID("plugin_pdf_opds_authfeed_title"));
        b.setMessage(EUExUtil.getResStringID("plugin_pdf_opds_authfeed_msg"));
        b.setView(view);
        b.setPositiveButton(  EUExUtil.getResStringID("plugin_pdf_opds_authfeed_ok"), EUExUtil.getResIdID("actions_setFeedAuth"), new EditableValue("username",
                editUsername), new EditableValue("password", editPassword), new Constant("info", info));
...
//            actions.getOrCreateAction(R.id.opdsrefreshfolder).run();
            actions.getOrCreateAction(EUExUtil.getResIdID("opdsrefreshfolder")).run();
...
//            info = (TextViewMultilineEllipse) convertView.findViewById(R.id.opdsDescription);
            ...
            textView = (TextView) convertView.findViewById( EUExUtil.getResIdID("opdsItemText"));
            imageView = (ImageView) convertView.findViewById( EUExUtil.getResIdID("opdsItemIcon"));
//            info = (TextViewMultilineEllipse) convertView.findViewById( EUExUtil.getResIdID("opdsDescription"));
