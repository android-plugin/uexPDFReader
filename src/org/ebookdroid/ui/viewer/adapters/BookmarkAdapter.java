...
//        this.end = new Bookmark(true, context.getString(R.string.bookmark_end), PageIndex.LAST, 0, 1);
        ...
        this.start = new Bookmark(true, context.getString(  EUExUtil.getResStringID("plugin_pdf_bookmark_start")), PageIndex.FIRST, 0, 0);
        this.end = new Bookmark(true, context.getString(  EUExUtil.getResStringID("plugin_pdf_bookmark_end")), PageIndex.LAST, 0, 1);
    ...
    public View getView(final int index, View itemView, final ViewGroup parent) {
        if (itemView == null) {
//            itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.plugin_pdf_bookmark, parent, false);
//
...
//            bar.setProgressDrawable(context.getResources().getDrawable(R.drawable.plugin_pdf_viewer_goto_dlg_progress));
        	
        	  itemView = LayoutInflater.from(parent.getContext()).inflate(EUExUtil.getResLayoutID("bookmark"), parent, false);

              final BookmarkView text = (BookmarkView) itemView.findViewById( EUExUtil.getResIdID("bookmarkName"));
              text.setActions(actions);

              final ProgressBar bar = (ProgressBar) itemView.findViewById( EUExUtil.getResIdID("bookmarkPage"));
              bar.setProgressDrawable(context.getResources().getDrawable( EUExUtil.getResDrawableID("plugin_pdf_viewer_goto_dlg_progress")));
        }

        final Bookmark b = getBookmark(index);

        final TextView text = (TextView) itemView.findViewById( EUExUtil.getResIdID("bookmarkName"));
        text.setText(b.name);
        text.setTag(b);
...
//        final View btn = itemView.findViewById(R.id.bookmark_remove);
        final View btn = itemView.findViewById( EUExUtil.getResIdID("bookmark_remove"));
        if (b.service) {
            btn.setVisibility(View.GONE);
        } else {
            btn.setVisibility(View.VISIBLE);
//            btn.setOnClickListener(actions.getOrCreateAction(R.id.actions_showDeleteBookmarkDlg));
            btn.setOnClickListener(actions.getOrCreateAction( EUExUtil.getResIdID("actions_showDeleteBookmarkDlg")));
            btn.setTag(b);
        }
