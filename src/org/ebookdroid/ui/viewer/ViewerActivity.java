...
public class ViewerActivity extends
		AbstractActionActivity<ViewerActivity, ViewerActivityController> {
	...
	protected void onCreateImpl(final Bundle savedInstanceState) {
		...
		try {
			...
			getTouchView().setOnClickListener(new OnClickListener() {
			...
			frameLayout.addView(getBackView(),lp);

		} catch (final Throwable th) {
			final ActionDialogBuilder builder = new ActionDialogBuilder(this,
					getController());
			// builder.setTitle(R.string.error_dlg_title);
			// builder.setMessage(th.getMessage());
			// builder.setPositiveButton(R.string.error_close,
			// R.id.mainmenu_close);
			// builder.show();
			...
			builder.setPositiveButton(EUExUtil.getResStringID("plugin_pdf_error_close"),
					EUExUtil.getResStringID("plugin_pdf_mainmenu_close"));
			builder.show();
		}
	...
	public void currentPageChanged(final String pageText, final String bookTitle) {
		...
		if (pageNumberToast != null) {
			...
			pageNumberToast = Toast
					.makeText(this, pageText, Toast.LENGTH_SHORT);
		}
	...
	public PageViewZoomControls getZoomControls() {
		if (zoomControls == null) {
			zoomControls = new PageViewZoomControls(this, getController()
					.getZoomModel());
	...
	public void onCreateContextMenu(final ContextMenu menu, final View v,
		...
		// menu.setHeaderIcon(R.drawable.plugin_pdf_application_icon);
		// final MenuInflater inflater = getMenuInflater();
		// inflater.inflate(R.menu.mainmenu_context, menu);
		// updateMenuItems(menu);

		menu.clear();
		menu.setHeaderTitle(EUExUtil.getResStringID("plugin_pdf_app_name"));
		menu.setHeaderIcon(EUExUtil.getResDrawableID("plugin_pdf_application_icon"));
		final MenuInflater inflater = getMenuInflater();
		inflater.inflate(ResUtils.getResMenuId("plugin_pdf_mainmenu_context"), menu);
		updateMenuItems(menu);
	}
	...
	protected boolean hasNormalMenu() {
		return AndroidVersion.lessThan4x
				|| IUIManager.instance.isTabletUi(this)
	...
	//
	// ActionMenuHelper.setMenuItemChecked(menu, as.fullScreen,
	// R.id.mainmenu_fullscreen);
	//
	// if (!AndroidVersion.lessThan3x) {
	// ActionMenuHelper.setMenuItemChecked(menu, as.showTitle,
	// R.id.mainmenu_showtitle);
	// } else {
	// ActionMenuHelper.setMenuItemVisible(menu, false,
	// R.id.mainmenu_showtitle);
	// }
	//
	// ActionMenuHelper
	// .setMenuItemChecked(menu, getZoomControls().getVisibility() ==
	// View.VISIBLE, R.id.mainmenu_zoom);
	//
	...
	// }
	//
	// ActionMenuHelper.setMenuItemChecked(menu, bs.rotation ==
	// BookRotationType.PORTRAIT,
	// R.id.mainmenu_force_portrait);
	// ActionMenuHelper.setMenuItemChecked(menu, bs.rotation ==
	// BookRotationType.LANDSCAPE,
	// R.id.mainmenu_force_landscape);
	// ActionMenuHelper.setMenuItemChecked(menu, bs.nightMode,
	// R.id.mainmenu_nightmode);
	// ActionMenuHelper.setMenuItemChecked(menu, bs.cropPages,
	// R.id.mainmenu_croppages);
	// ActionMenuHelper.setMenuItemChecked(menu, bs.splitPages,
	// R.id.mainmenu_splitpages,
	// R.drawable.plugin_pdf_viewer_menu_split_pages,
	// R.drawable.plugin_pdf_viewer_menu_split_pages_off);
	//
	// final DecodeService ds = getController().getDecodeService();
	//
	// final boolean cropSupported =
	// ds.isFeatureSupported(CodecFeatures.FEATURE_CROP_SUPPORT);
	// ActionMenuHelper.setMenuItemVisible(menu, cropSupported,
	// R.id.mainmenu_croppages);
	// ActionMenuHelper.setMenuItemVisible(menu, cropSupported,
	// R.id.mainmenu_crop);
	//
	// final boolean splitSupported =
	// ds.isFeatureSupported(CodecFeatures.FEATURE_SPLIT_SUPPORT);
	// ActionMenuHelper.setMenuItemVisible(menu, splitSupported,
	// R.id.mainmenu_splitpages);
	//
	...
	// LengthUtils.isNotEmpty(bs.bookmarks)) {
	...
	// final MenuItem bmi = menu.add(R.id.actions_goToBookmarkGroup,
	// R.id.actions_goToBookmark, Menu.NONE, b.name);
	...
	public void showToastText(final int duration, final int resId,
			final Object... args) {
		Toast.makeText(getApplicationContext(),
				getResources().getString(resId, args), duration).show();
	}
	...
	public void setEUExPdf(EUExPdf pdfContext) {
		this.pdfContext = pdfContext;
	}

	public View getBackView() {
		RelativeLayout parent = (RelativeLayout) LayoutInflater.from(ViewerActivity.this).inflate(EUExUtil.getResLayoutID("plugin_pdf_title_back"), null);
		Button back = (Button) parent.findViewById(EUExUtil.getResIdID("plugin_pdf_top_back_button"));
		
		back.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				//getEUExPdf().closeWindow();
				finish();
