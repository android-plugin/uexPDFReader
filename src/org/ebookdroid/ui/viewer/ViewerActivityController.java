...
public class ViewerActivityController extends
		AbstractActivityController<ViewerActivity> implements
		IActivityController, DecodingProgressListener, CurrentPageListener,
		IAppSettingsChangeListener, IBookSettingsChangeListener {

	private final AtomicReference<IViewController> ctrl = new AtomicReference<IViewController>(
			ViewContollerStub.STUB);
	...
	public ViewerActivityController(final ViewerActivity activity) {
		super(activity, BEFORE_CREATE, BEFORE_RECREATE, AFTER_CREATE,
				ON_POST_CREATE, ON_DESTROY);
	 ...
	 */
	@Override
	public void afterCreate(final ViewerActivity activity,
			final boolean recreated) {

		final AppSettings appSettings = AppSettings.current();

		IUIManager.instance.setFullScreenMode(activity,
				getManagedComponent().view.getView(), appSettings.fullScreen);

		// createAction(R.id.mainmenu_crop).putValue("view",
		// activity.getManualCropControls()).putValue("mode",
		// DocumentViewMode.SINGLE_PAGE);
		// createAction(R.id.mainmenu_zoom).putValue("view",
		// activity.getZoomControls());
		// createAction(R.id.mainmenu_search).putValue("view",
		// activity.getSearchControls());
		// createAction(R.id.actions_toggleTouchManagerView).putValue("view",
		// activity.getTouchView());
		// createAction(R.id.mainmenu_force_portrait).putValue("mode",
		// BookRotationType.PORTRAIT);
		// createAction(R.id.mainmenu_force_landscape).putValue("mode",
		// BookRotationType.LANDSCAPE);
		...
		if (intent == null) {
			...
			showErrorDlg(EUExUtil.getResStringID("plugin_pdf_msg_bad_intent"), intent);
			return;
		}
		...
		if (data == null) {
			...
			showErrorDlg(EUExUtil.getResStringID("plugin_pdf_msg_no_intent_data"), intent);
			return;
		}
		...
		if (codecType == null) {
			...
			showErrorDlg(
					EUExUtil.getResStringID("plugin_pdf_msg_unknown_intent_data_type"),
					intent);
			return;
		}
		...
		if (scheme.temporary) {
			...
			m_fileName = PathFromUri.retrieve(activity.getContentResolver(),
					uri);
		}

		bookSettings = SettingsManager.create(id, m_fileName, scheme.temporary,
				intent);
	...
	public void askPassword(final String fileName, final int promtId) {
		...
		final ActionDialogBuilder builder = new ActionDialogBuilder(
				getManagedComponent(), this);
		builder.setTitle(fileName).setMessage(promtId).setView(input);
		// builder.setPositiveButton(R.id.actions_redecodingWithPassword,
		// new EditableValue("input", input));
		// builder.setNegativeButton(R.id.mainmenu_close).show();

		builder.setPositiveButton(
				EUExUtil.getResIdID("actions_redecodingWithPassword"),
				new EditableValue("input", input));
		builder.setNegativeButton(EUExUtil.getResIdID("mainmenu_close")).show();
	...
	public void showErrorDlg(final int msgId, final Object... args) {
		...
		builder.setTitle(EUExUtil.getResStringID("plugin_pdf_error_dlg_title"));
		builder.setMessage(msgId, args);
		builder.setPositiveButton(EUExUtil.getResStringID("plugin_pdf_error_close"),
				EUExUtil.getResIdID("mainmenu_close"));
		builder.show();
	...
	public void currentPageChanged(final PageIndex oldIndex,
		...
		final Runnable r = new Runnable() {
			...
			public void run() {
				...
				String pageText = "";
				if (pageCount > 0) {
					final int offset = bookSettings != null ? bookSettings.firstPageOffset
							: 1;
					if (offset == 1) {
						pageText = (newIndex.viewIndex + 1) + "/" + pageCount;
					} else {
						pageText = offset + "/" + (newIndex.viewIndex + offset)
								+ "/" + (pageCount - 1 + offset);
					}
				}
				getManagedComponent().currentPageChanged(pageText, bookTitle);
				SettingsManager.currentPageChanged(bookSettings, oldIndex,
						newIndex);
			}
		};
	...
	// IUIManager.instance.openOptionsMenu(getManagedComponent(),
	...
	public void gotoOutlineItem(final ActionEx action) {
		final OutlineLink link = action
				.getParameter(IActionController.ADAPTER_SELECTED_ITEM_PROPERTY);
		if (link == null) {
			return;
		}
		...
		if (link.targetPage != -1) {
			...
			if (link.targetPage < 1 || link.targetPage > pageCount) {
				// getManagedComponent().showToastText(2000,
				// R.string.error_page_out_of_rande, pageCount);
				getManagedComponent().showToastText(2000,
						EUExUtil.getResStringID("plugin_pdf_error_page_out_of_rande"),
						pageCount);
			} else {
				getDocumentController().goToLink(link.targetPage - 1,
						link.targetRect,
	 ...
	 */
	@Override
	public void jumpToPage(final int viewIndex, final float offsetX,
			final float offsetY, final boolean addToHistory) {
		if (addToHistory) {
			history.update();
		}
	...
	public void showOutline(final ActionEx action) {
		// final List<OutlineLink> outline = documentModel.decodeService
		// .getOutline();
		// if ((outline != null) && (outline.size() > 0)) {
		// final OutlineDialog dlg = new OutlineDialog(this, outline);
		// dlg.show();
		// } else {
		// getManagedComponent().showToastText(Toast.LENGTH_SHORT,
		// R.string.outline_missed);
		// }
	}
	...
	public final void doSearch(final ActionEx action) {
		...
		final String newPattern = (value != null ? value.toString()
				: LengthUtils.toString(action.getParameter("text")));
		final String oldPattern = currentSearchPattern;

		currentSearchPattern = newPattern;

		executor.execute(new SearchTask(), newPattern, oldPattern,
				(String) action.getParameter("forward"));
	}
	...
	public void showBookSettings(final ActionEx action) {
		SettingsUI.showBookSettings(getManagedComponent(),
				bookSettings.fileName);
	}

	// @ActionMethod(ids = R.id.mainmenu_settings)
	public void showAppSettings(final ActionEx action) {
		SettingsUI
				.showAppSettings(getManagedComponent(), bookSettings.fileName);
	}
	...
	public void forceOrientation(final ActionEx action) {
		...
		if (bookSettings.rotation == mode) {
			SettingsManager.setBookRotation(bookSettings,
	...
	public void showBookmarkDialog(final ActionEx action) {
		...
		// final String message = getManagedComponent().getString(
		// R.string.add_bookmark_name);
		final String message = getManagedComponent().getString(
				EUExUtil.getResStringID("plugin_pdf_add_bookmark_name"));

		final BookSettings bs = getBookSettings();
		final int offset = bs != null ? bs.firstPageOffset : 1;

		// final EditText input = (EditText) LayoutInflater.from(
		// getManagedComponent()).inflate(R.layout.plugin_pdf_bookmark_edit, null);
		// input.setText(getManagedComponent().getString(R.string.text_page) +
		// " "
		// + (page + offset));
		final EditText input = (EditText) LayoutInflater.from(
				getManagedComponent()).inflate(
				EUExUtil.getResLayoutID("plugin_pdf_bookmark_edit"), null);
		input.setText(getManagedComponent().getString(
				EUExUtil.getResStringID("plugin_pdf_text_page"))
				+ " " + (page + offset));
		...
		// .setView(input);
		// builder.setPositiveButton(R.id.actions_addBookmark, new
		// EditableValue(
		// "input", input));
		builder.setTitle(EUExUtil.getResStringID("plugin_pdf_menu_add_bookmark"))
				.setMessage(message).setView(input);
		builder.setPositiveButton(
				EUExUtil.getResStringID("plugin_pdf_actions_addBookmark"),
				new EditableValue("input", input));
	...
	public void addBookmark(final ActionEx action) {
		...
		if (page != null) {
			...
			bookSettings.bookmarks.add(new Bookmark(name, documentModel
					.getCurrentIndex(), pos.x, pos.y));
	...
	public void goToBookmark(final ActionEx action) {
		...
		final Page actualPage = b.page.getActualPage(getDocumentModel(),
				bookSettings);
		if (actualPage != null) {
			jumpToPage(actualPage.index.viewIndex, b.offsetX, b.offsetY,
	...
	// R.id.actions_toggleTouchManagerView, R.id.mainmenu_search,
	// R.id.mainmenu_crop })
	public void toggleControls(final ActionEx action) {
		...
		if (mode != null && bookSettings != null
				&& bookSettings.viewMode != mode) {

			final ActionDialogBuilder builder = new ActionDialogBuilder(
					getContext(), this);
			builder.setTitle(android.R.string.dialog_alert_title);
			// builder.setMessage(R.string.error_invalid_view_mode,
			// mode.getResValue());
			builder.setMessage(
					EUExUtil.getResStringID("plugin_pdf_error_invalid_view_mode"),
					mode.getResValue());
...
//				toggleControls(getAction(R.id.mainmenu_search));
				toggleControls(getAction(EUExUtil.getResIdID("mainmenu_search")));
		...
		if (action == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_BACK) {
			if (event.getRepeatCount() == 0) {
				if (getManagedComponent().getTouchView().isShown()) {
					ViewEffects.toggleControls(getManagedComponent()
							.getTouchView());
				} else if (getManagedComponent().getManualCropControls()
						.isShown()) {
					ViewEffects.toggleControls(getManagedComponent()
							.getManualCropControls());
				} else {
...
//						builder.setPositiveButton(R.id.mainmenu_close);
						...
						builder.setTitle(EUExUtil.getResStringID("plugin_pdf_confirmclose_title"));
						builder.setMessage(EUExUtil.getResStringID("plugin_pdf_confirmclose_msg"));
						builder.setPositiveButton(EUExUtil.getResStringID("plugin_pdf_mainmenu_close"));
						builder.setNegativeButton().show();
						
						 
					} else {
//						getOrCreateAction(R.id.mainmenu_close).run();
						getOrCreateAction(EUExUtil.getResIdID("mainmenu_close")).run();
...
//			getOrCreateAction(R.id.actions_doClose).run();
			getOrCreateAction(EUExUtil.getResIdID("actions_doClose")).run();
			return;
		}

//		getOrCreateAction(R.id.actions_doSaveAndClose).putValue("save",
//				Boolean.TRUE);
...
//		builder.setPositiveButton(R.string.confirmsave_yes_btn,
//				R.id.actions_showSaveDlg);
//		builder.setNegativeButton(R.string.confirmsave_no_btn,
//				R.id.actions_doClose);
//		builder.show();
		
		getOrCreateAction(EUExUtil.getResIdID("actions_doSaveAndClose")).putValue("save",
				Boolean.TRUE);
		getOrCreateAction(EUExUtil.getResIdID("actions_doClose")).putValue("save", Boolean.FALSE);

		final ActionDialogBuilder builder = new ActionDialogBuilder(
				getManagedComponent(), this);
		builder.setTitle(EUExUtil.getResStringID("plugin_pdf_confirmclose_title"));
		builder.setMessage(EUExUtil.getResStringID("plugin_pdf_confirmsave_msg"));
		builder.setPositiveButton(EUExUtil.getResStringID("plugin_pdf_confirmsave_yes_btn"),
				EUExUtil.getResIdID("actions_showSaveDlg"));
		builder.setNegativeButton(EUExUtil.getResStringID("plugin_pdf_confirmsave_no_btn"),
				EUExUtil.getResIdID("actions_doClose"));
		builder.show();
...
//		dlg.show(BaseDroidApp.EXT_STORAGE, R.string.confirmclose_title,
//				R.id.actions_doSaveAndClose, R.id.actions_doClose);
		
		dlg.show(BaseDroidApp.EXT_STORAGE, EUExUtil.getResStringID("plugin_pdf_confirmclose_title"),
				EUExUtil.getResIdID("actions_doSaveAndClose"), EUExUtil.getResIdID("actions_doClose"));
...
//	@ActionMethod(ids = R.id.actions_doSaveAndClose)
	public void doSaveAndClose(final ActionEx action) {
		final File targetFolder = action
				.getParameter(FolderDlg.SELECTED_FOLDER);
		...
		try {
			...
			CacheManager.copy(source.getAbsolutePath(),
...
//	@ActionMethod(ids = R.id.actions_doClose)
	...
	public void onAppSettingsChanged(final AppSettings oldSettings,
		...
		if (diff.isRotationChanged()) {
			if (bookSettings != null) {
				activity.setRequestedOrientation(bookSettings
						.getOrientation(newSettings));
			} else {
				activity.setRequestedOrientation(newSettings.rotation
						.getOrientation());
			}
		}
		...
		if (diff.isFullScreenChanged()) {
			IUIManager.instance.setFullScreenMode(activity,
					activity.view.getView(), newSettings.fullScreen);
		}

		if (!diff.isFirstTime() && diff.isShowTitleChanged()) {
			IUIManager.instance.setTitleVisible(activity,
	...
	public void onBookSettingsChanged(final BookSettings oldSettings,
		...
		if (diff.isRotationChanged()) {
			getManagedComponent().setRequestedOrientation(
					newSettings.getOrientation(AppSettings.current()));
		}
	...
	final class BookLoadTask extends BaseAsyncTask<String, Throwable> implements
			Runnable, IProgressIndicator {

		private final String m_password;

		public BookLoadTask(final String password) {
//			super(getManagedComponent(), R.string.msg_loading, false);
			super(getManagedComponent(), EUExUtil.getResStringID("plugin_pdf_msg_loading"), false);
			m_password = password;
		}
		...
		protected void onPostExecute(Throwable result) {
			...
			try {
				...
				if (result instanceof MuPdfPasswordException) {
					...
					final int promptId = pex.isWrongPasswordEntered() ? EUExUtil.getResStringID("plugin_pdf_msg_wrong_password")
							: EUExUtil.getResStringID("plugin_pdf_msg_password_required");
...
//					showErrorDlg(R.string.msg_unexpected_error, msg);
					showErrorDlg(EUExUtil.getResStringID("plugin_pdf_msg_unexpected_error"), msg);
	...
	final class SearchTask extends AsyncTask<String, String, RectF> implements
			SearchModel.ProgressCallback, OnCancelListener {
		...
		public void searchStarted(final int pageIndex) {
			final int offset = bookSettings != null ? bookSettings.firstPageOffset
					: 1;
			publishProgress(getManagedComponent().getResources().getString(
					EUExUtil.getResStringID("plugin_pdf_msg_search_text_on_page"), pageIndex + offset));
		...
		protected RectF doInBackground(final String... params) {
			try {
				...
				final boolean forward = length >= 3 ? Boolean
						.parseBoolean(params[2]) : true;

				searchModel.setPattern(pattern);

				final RectF current = forward ? searchModel.moveToNext(this)
						: searchModel.moveToPrev(this);
				targetPage = searchModel.getCurrentPage();
				if (LCTX.isDebugEnabled()) {
					LCTX.d("SearchTask.doInBackground(): " + targetPage + " "
							+ current);
				}
				return current;
		...
		protected void onPostExecute(final RectF result) {
			...
			if (result != null) {
				...
				final SearchControls sc = getManagedComponent()
						.getSearchControls();
				final int controlsHeight = 3 + sc.getActualHeight();
				final float pageHeight = targetPage.getBounds(
						getZoomModel().getZoom()).height();
				newRect.offset(0, -(controlsHeight / pageHeight));
				getDocumentController().goToLink(targetPage.index.docIndex,
						newRect, AppSettings.current().storeSearchGotoHistory);
			} else {
				Toast.makeText(getManagedComponent(),
						EUExUtil.getResStringID("plugin_pdf_msg_no_text_found"), Toast.LENGTH_LONG).show();
			}
		...
		protected void onProgressUpdate(final String... values) {
			...
			if (progressDialog == null || !progressDialog.isShowing()) {
				progressDialog = ProgressDialog.show(getManagedComponent(), "",
						last, true);
